...
void WorkerManage::AddEmp() {
  cout << "输入添加职工的数量：" << endl;
  int add_Empnum = 0; //存储用户的输入
  cin >> add_Empnum;
  if (add_Empnum > 0) {
    //计算添加后所需空间的大小
    int newSize = this->m_EmpNum + add_Empnum;//原来的+新添加的
    ...
    //提示
    cout << "成功添加" << add_Empnum << "个新职工" << endl;
    //保存数据到文件中
    this->SaveFile();
  } else {
    cout << "输入有误" << endl;
...
void WorkerManage::Mod_Emp() {
  if (this->m_FileIsEmpty) {
    ...
    if (ret != -1) {
      ...
      Worker *worker = NULL;
      switch (newselect) {
        case 1:worker = new Employee(newid, newname, newselect);
          break;
        case 2:worker = new Manager(newid, newname, newselect);
          break;
        case 3:worker = new Boss(newid, newname, newselect);
          break;
        default:break;
      }
      //更新数据到数组中
